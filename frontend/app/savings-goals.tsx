...
export default function SavingsGoals() {
  ...
  const handleDeposit = async () => {
    ...
    if (amt <= 0 || !depositAccountId) { Alert.alert('Blad', 'Podaj kwote i wybierz konto'); return; }
    try {
      await savingsGoalsDB.addAmount(depositGoalId, amt);
      await transactionsDB.create({
        type: 'expense', amount: amt, category: 'Oszczednosci',
        account_id: depositAccountId, date: new Date().toISOString(),
        description: `Wplata na cel: ${depositGoalName}`, is_transfer: false,
      });
      setDepositModal(false);
      fetch_();
      Alert.alert('Gotowe', `Wplacono ${amt.toFixed(2)} PLN na "${depositGoalName}"`);
    } catch (e) { Alert.alert('Blad', 'Nie udalo sie wykonac wplaty'); }
  };
  ...
  return (
    <View style={s.container}>
      <View style={s.headerBar}>
        ...
        <Text style={s.headerTitle}>Cele Oszczednosciowe</Text>
        <View style={{ width: 24 }} />
      </View>
          ...
          return (
            <View style={s.emergencyCard}>
              ...
              <Text style={{fontSize:13,color:'#6B5D52',marginTop:4}}>
                {avgMonthlyExpense > 0 ? `Pokrywa ${monthsCovered.toFixed(1)} miesiaca wydatkow (sr. ${avgMonthlyExpense.toFixed(0)} PLN/mies.)` : 'Dodaj transakcje aby obliczyc pokrycie'}
              </Text>
              ...
              <Text style={{fontSize:11,color:'#9B8B7E',marginTop:4}}>Cel: 6 miesiecy wydatkow</Text>
              {emergencyGoals.length > 0 && (
                <TouchableOpacity style={s.emergencyDepositBtn} onPress={() => openDeposit(emergencyGoals[0].id, 'Fundusz Awaryjny')}>
                  ...
                  <Text style={s.emergencyDepositText}>Wplac</Text>
                </TouchableOpacity>
              )}
            </View>
          );
        })()}
        ListEmptyComponent={<View style={s.empty}><Ionicons name="trophy-outline" size={64} color="#9B8B7E" /><Text style={s.emptyText}>Brak celow</Text>
          ...
          return (
            <View style={s.card}>
              <View style={s.cardHeader}>
                ...
                <TouchableOpacity onPress={() => Alert.alert('Usun', `Usunac cel "${item.name}"?`, [{ text: 'Anuluj' }, { text: 'Usun', style: 'destructive', onPress: async () => { await savingsGoalsDB.delete(item.id); fetch_(); } }])}>
              ...
              <Text style={s.pctText}>{pct.toFixed(0)}% osiagniete</Text>
              <TouchableOpacity style={s.depositBtn} onPress={() => openDeposit(item.id, item.name)}>
                <Ionicons name="wallet" size={18} color="#FFF" />
                <Text style={s.depositBtnText}>Wplac</Text>
              </TouchableOpacity>
        ...
        <View style={s.modalOverlay}>
          <View style={s.modalContent}>
            <View style={s.modalHeader}>
              <Text style={s.modalTitle}>Wplac na: {depositGoalName}</Text>
            ...
            <TouchableOpacity style={[s.submitBtn, (!depositAmount || !depositAccountId) && {opacity:0.5}]} onPress={handleDeposit} disabled={!depositAmount || !depositAccountId}>
              <Text style={s.submitBtnText}>Wplac</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
